    ...
    <div className="min-h-screen bg-[#fafafa] text-slate-800 font-sans antialiased selection:bg-slate-900 selection:text-white pb-16">
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-2xs">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          ...
          <nav className="flex items-center space-x-1 sm:space-x-1.5 overflow-x-auto py-1.5 no-scrollbar max-w-full">
            {topNavTabs.map((tab) => {
              const Icon = tab.icon;
              ...
              return (
                <button
                  ...
                  className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all shrink-0 active:scale-95 touch-manipulation ${
                    isActive
              ...
              <div className="space-y-6 animate-fadeIn">
                ...
                <div className="space-y-5">
                  {researchPapers.map((paper) => (
                    <div key={paper.id} className="bg-white rounded-2xl border border-slate-200/90 p-5 space-y-3.5 shadow-2xs hover:border-slate-300 transition-all">
                      
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-3 border-b border-slate-100">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3.5">
                          {paper.publisherLogo && (
                            <div className="h-16 sm:h-20 w-36 sm:w-44 px-3 py-2 rounded-xl bg-white border border-slate-200/90 flex items-center justify-center shrink-0 shadow-xs hover:scale-102 transition-transform">
                              <img 
                                src={paper.publisherLogo} 
                                alt={paper.venue} 
                          ...
                          <div className="space-y-1">
                            <span className="text-[10px] font-extrabold uppercase px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 inline-block">
                              [{paper.status}] {paper.venue}
                            </span>
                            <h3 className="text-sm sm:text-base font-bold text-slate-900 leading-snug">
                              {paper.title}
                            </h3>
                            <p className="text-xs font-semibold text-slate-400">📍 {paper.location}</p>
                          </div>
                        </div>
                        ...
                        {paper.link && (
                          <a 
                            ...
                            target="_blank" 
                            rel="noreferrer"
                            className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 text-xs font-bold shrink-0 transition-colors self-start sm:self-auto"
                      ...
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        {paper.tags.map((t, idx) => (
                          <span key={idx} className="text-[10px] font-semibold px-2.5 py-0.5 rounded-md bg-slate-100 text-slate-600 border border-slate-200/60">
                            #{t}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="pt-2 space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {honorsAndHackathons.map((item, idx) => (
                        <div key={idx} className="p-3.5 rounded-xl bg-white border border-slate-200/80 space-y-1 text-xs flex items-center space-x-3.5 shadow-2xs">
                          {item.logo && (
                            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-white border border-slate-200/90 p-1.5 flex items-center justify-center shrink-0 shadow-2xs">
